...
<table class="shop_table shop_table_responsive my_account_orders woocommerce-orders-table woocommerce-MyAccount-orders woocommerce-orders-table--orders">
	...
	<tbody>
		...
		<?php foreach ( $subscription_orders as $subscription_order ) :
            ...
            $listedOrder[$order->get_id()] = $order->get_id();
			        ...
			        if (  function_exists( 'loadKonnektiveOrder' ) ) {
    			        ...
    			        if ($purchases['result'] == 'SUCCESS' && !empty($purchases['message'])) {
                            if (isset($purchases['message']['data']) && !empty($purchases['message']['data'])) {
                                if(!empty($purchases['message']['data'][0]['purchaseId'])){
         
                                            $transactions = queryKKTransactions($purchases['message']['data'][0]['purchaseId']);

                        			        if ($transactions['result'] == 'SUCCESS' && !empty($transactions['message'])) {
                                                if (isset($transactions['message']['data']) && !empty($transactions['message']['data'])) {
                                                    if(!empty($transactions['message']['data'])){
                                                        
                                                         foreach ($transactions['message']['data'] as $transaction) {
                    
                                                            $existingOrders = wc_get_orders( array(
                                                                'limit'        => -1, // Query all orders
                                                                'orderby'      => 'date',
                                                                'order'        => 'DESC',
                                                                'meta_key'     => 'order_konnektive_order_id',
                                                                'meta_value'    => $transaction['clientOrderId'],
                                                                'meta_compare' => "="
                                                            ));
                                                            ...
                                                            foreach($existingOrders as $or) {
                                                                if(!isset($listedOrder[$or->get_id()])) {
                                                                    $kk_orders[] = $or->get_id();
                                                                }
                                                            }
                                                         }
		...
		?>
		<?php foreach ( $kk_orders as $kk_order ) :
			$order = wc_get_order( $kk_order );

			if ( ! $order || isset($listedOrder[$order->get_id()])) {
				continue;
			}
